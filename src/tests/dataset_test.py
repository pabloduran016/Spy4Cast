...
class DatasetTest(BaseTestCase):
    def test_from_xrarray(self) -> None:
        ...
        xr_ds = xr.open_dataset(os.path.join(DATASETS_FOLDER, HadISST_sst))
    ...
    def test_get_data(self) -> None:
        ds = Dataset(HadISST_sst, DATASETS_FOLDER)
    ...
    def test_set_data(self) -> None:
        ds0 = Dataset(HadISST_sst, DATASETS_FOLDER).open(SST)
        ds = Dataset(HadISST_sst, DATASETS_FOLDER)
        self.assertNotHasAttr(ds, '_data')
        ds.data = ds0.data
        self.assertHasAttr(ds, '_data')
    ...
    def test_get_time(self) -> None:
        ds = Dataset(HadISST_sst, DATASETS_FOLDER).open(SST)
    ...
    def test_get_timestamp0(self) -> None:
        ...
        ds2 = Dataset(chlos_bscr_1958_2016, DATASETS_FOLDER).open(CHLOS)
    ...
    def test_open(self) -> None:
        ...
        _ = Dataset(oisst_v2_mean_monthly, DATASETS_FOLDER).open()  # Decode times.open(
        ...
        with self.assertRaises(DatasetNotFoundError):
            _ = Dataset('non_existing_dataset.nc', DATASETS_FOLDER).open()
    ...
    def test_slice(self) -> None:
        for region, year0, yearf, month0, monthf in (
            ...
            print(region)
            ds = Dataset(HadISST_sst, DATASETS_FOLDER).open(SST).slice(region)
    ...
    def test_save_nc(self) -> None:
        ...
        ds = Dataset(HadISST_sst, DATASETS_FOLDER).open(SST).slice(sl)
        ds.save_nc(name)
