...
class Spy4Caster:
    ...
    def plot_preprocessed(self, flags: int = 0, fig: plt.Figure = None,
        ...
        fig.suptitle(f'Y: {slise2str(self._rdy._slise)}, '
    ...
    def plot_crossvalidation(self, flags: int = 0,
        ...
        hatches = d.copy()
        hatches[
            (p_z_zhat_s > alpha).transpose().reshape((nzlat, nzlon))
        ] = np.nan
        axs[0].contourf(
            zlons, zlats, hatches,
        ...
        axs[1].scatter(
            ts[p_z_zhat_t <= alpha], r_z_zhat_t[p_z_zhat_t <= alpha]
        )
        ...
        for mode in range(mean.shape[0]):
            ...
            axs[3 + mode].bar(
                ts, mx[mode] - mn[mode], bottom=mn[mode], color='purple',
                width=.2
            )
            axs[3 + mode].plot(
                ts, mean[mode], label='Mean', color='orange', linewidth=3
            )
            axs[3 + mode].plot(
                ts, mx[mode], label='Mean', color='r', linewidth=.5, alpha=.5
            )
            axs[3 + mode].plot(
                ts, mn[mode], label='Mean', color='r', linewidth=.5, alpha=.5
            )
    ...
    def run(self, flags: int = 0, **kwargs: Any) -> 'Spy4Caster':
        ...
        if F.SHOW_PLOT in flags or F.SAVE_FIG in flags:
            try:
                self.plot_mca(
                    flags & ~F.SHOW_PLOT | F.NOT_HALT, cmap=cmap
                )
                self.plot_crossvalidation(
                    flags & ~F.SHOW_PLOT | F.NOT_HALT, cmap=cmap
                )
                self.plot_zhat(
                    flags & ~F.SHOW_PLOT | F.NOT_HALT, sy=sy, cmap=cmap
                )
