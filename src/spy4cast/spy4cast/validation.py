...
class Validation(_Procedure):
    ...
    def plot_zhat(
        ...
        for i, year in enumerate(years):
            ...
            im = plot_map(d0[yindex], self._validating_dsy.lat, self._validating_dsy.lon, fig, ax0, f'$Y({y_year})$', ticks=y_ticks, xlim=y_xlim,
            ...
            plot_map(
                d1[zindex], self._validating_dsz.lat, self._validating_dsz.lon, fig, ax1, rf'$\hat Z({year})$',
            ...
            im = plot_map(
                d2[zindex], self._validating_dsz.lat, self._validating_dsz.lon, fig, ax2, f'Z({year})',
                cmap=cmap, levels=levels, ticks=z_ticks, xlim=z_xlim,
                add_cyclic_point=self._validating_dsz.region.lon0 >= self._validating_dsz.region.lonf, plot_type=plot_type,
                colorbar=False,
            )
